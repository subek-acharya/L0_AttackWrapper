...
def L0_PGD_AttackWrapper(model, device, dataLoader, n_restarts, num_steps, step_size, sparsity):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(dataLoader):
        ...
        adv_batch, pgd_adv_acc_batch = perturb(model, device, x_numpy, y_numpy, sparsity, num_steps, step_size, n_restarts)
    ...
    print(f"Overall Robust Accuracy: {overall_robust_acc:.2f}%")
...
def perturb(model, device, x_nat, y_nat, sparsity, num_steps, step_size, n_restarts):
    adv = np.copy(x_nat)
    pgd_adv_acc = None 
    ...
    for counter in range(n_restarts):
      ...
      x_batch_adv, curr_pgd_adv_acc = perturb_L0_box(model, x_nat, y_nat, -x_nat, 1.0 - x_nat, sparsity, num_steps, step_size, device)
